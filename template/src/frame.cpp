...
// 缓冲里写东西。
//
// 因此备 FRAMES_IN_FLIGHT 套资源轮流使用：录第 N 帧用第 N%2 套，录第 N+1 帧
...
bool Application::create_frame_resources()
{
    ...
    // 一次分配 FRAMES_IN_FLIGHT 个，直接填进数组。
    // 命令缓冲不需要单独销毁，销毁命令池时会一起没（见 ~Application()）。
    VKX_CHECK(vkAllocateCommandBuffers(device, &alloc_info, command_buffers));

    for (uint32_t i = 0; i < FRAMES_IN_FLIGHT; ++i) {
        // image_available：交换链图像取到手了，可以开始往上面画。
        // 这是 GPU 内部的等待——提交命令时挂上它，GPU 会自己等。
        VkSemaphoreCreateInfo semaphore_info{};
        semaphore_info.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
        VKX_CHECK(vkCreateSemaphore(device, &semaphore_info, nullptr, &image_available[i]));

        // in_flight：这一套资源上一次提交的活干完了没有。这个要 CPU 来等。
        //
        // 建成「已触发」状态：第一帧时这套资源还没提交过任何东西，栅栏若是
        // 未触发的，draw_frame() 开头那句 vkWaitForFences 会一直等下去。
        VkFenceCreateInfo fence_info{};
        fence_info.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
        fence_info.flags = VK_FENCE_CREATE_SIGNALED_BIT;
        VKX_CHECK(vkCreateFence(device, &fence_info, nullptr, &in_flight[i]));
    }
