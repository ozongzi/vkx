...
// 在显卡的内存类型里，找一个既被 type_bits 允许、又满足 wanted 标志的。
//
...
//
// 两个输入的含义不同：
//   type_bits  驱动给出的限制，说明这个 buffer/image 只能放在哪几种内存上。
//             它是个位图，第 i 位为 1 表示第 i 种内存类型可用。
//             这个值来自 vkGetBufferMemoryRequirements。
//   wanted    调用方的要求，例如「CPU 得能映射」。
//
// 两边取交集，第一个满足的就是答案。
bool Application::find_memory_type(uint32_t type_bits, VkMemoryPropertyFlags wanted, uint32_t* out)
{
    ...
    report_error("找不到满足要求的显存类型（flags = 0x%x）", wanted);
    return false;
}
